...
The conventional remote control(RC) is typically used as a standard medium for viewers to interact with their TVs. However, the interaction by using the RC presents two notable problems: (1) to automatically identify the viewers who have possession of it; and (2) to capture viewers' interactions and the context in which they occur. As a consequence, the entire group of viewers (e.g., a family) is treated as a single viewer. As a result, the tastes that were inferred from the users in charge of the RC can be imposed on the tastes of the others.
