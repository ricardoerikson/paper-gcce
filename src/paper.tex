...
The web applications can be hosted in the AHI

The requests handling is carried out by using the front controller architectural design pattern~\cite{Buschmann2007}. It is employed to centralize requests processing in a single component, which facilitates the design and implementation of services for different clients. As a result, client applications developed for different operating systems and send requests to and receive response from a single URL.
